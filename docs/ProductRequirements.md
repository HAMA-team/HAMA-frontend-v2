...
- [ ] 사용자 메시지와 AI 답변이 구분되어 표시됩니다
- [ ] AI 답변은 Markdown 형식(헤딩, 리스트, 테이블, 코드블록)을 지원합니다
- [ ] 메시지 전송 중 로딩 상태가 표시됩니다
- [ ] AI 응답 대기 중 "생각하는 중..." 애니메이션이 표시됩니다
...
- [ ] AI 답변 내부에 Thinking 섹션이 포함됩니다 (Claude 스타일)
- [ ] Thinking은 기본적으로 접혀 있으며, 클릭하면 펼쳐집니다
- [ ] 에이전트 단계별로 아이콘과 설명이 표시됩니다:
    - `planner` → 📋 "계획 수립 중..."
    - `researcher` → 🔍 "데이터 수집 중..."
    - `strategy` → 💡 "전략 분석 중..."
- [ ] 실시간 스트리밍으로 진행 상황이 업데이트됩니다
...
- [ ] 모든 AI 답변 하단에 "Save as Artifact" 버튼이 표시됩니다
- [ ] 버튼 클릭 시 저장 성공 토스트가 표시됩니다 (3초)
- [ ] 토스트에 "Artifacts 페이지에서 보기" 링크가 포함됩니다
...
- [ ] Backend에서 `requires_approval: true` 신호를 보내면 우측에 승인 패널이 표시됩니다
- [ ] 패널은 사용자가 결정을 내릴 때까지 닫을 수 없습니다
- [ ] 패널에 다음 정보가 표시됩니다:
    - 주문 내역 (종목명, 종목코드, 금액, 수량)
    - 리스크 경고 (현재 비중, 예상 비중, 경고 문구)
    - 권장 대안 (선택 사항)
- [ ] 승인/거부 버튼이 명확하게 구분됩니다
...
- [ ] 포트폴리오 데이터를 시각화된 차트로 표시합니다
- [ ] 3가지 차트 타입을 지원합니다:
    - **트리맵** (기본): 종목별 비중을 면적으로 표시
    - **원 그래프**: 섹터별 비중
    - **막대 그래프**: 수익률 순위
- [ ] 차트 타입 전환 버튼이 제공됩니다
- [ ] 각 종목을 클릭하면 상세 정보가 표시됩니다
