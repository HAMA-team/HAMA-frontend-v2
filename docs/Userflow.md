...
#### 시나리오 A: Copilot → Pilot 변경 (저위험 자동 매매)

```
[My Page]
      ↓
[5단계 워크플로우 프로그레스 바]
 - 데이터 수집 → 분석 → 포트폴리오 → 리스크 → 매매
 - 현재: Copilot (Lv2)
   - 🔴 포트폴리오, 🔴 매매 HITL 필요
      ↓
[사용자: Pilot (Lv1) 선택]
      ↓
[변경 확인 다이얼로그]
 "Pilot 모드로 변경하면 저위험 매매가 자동 실행됩니다.
  계속하시겠습니까?"
      ↓
[확인 클릭]
      ↓
[API: POST /api/v1/user/automation-level] (Phase 2+)
      ↓
[토스트: "자동화 레벨이 변경되었습니다"]
      ↓
[워크플로우 프로그레스 바 업데이트]
 - Pilot (Lv1): 🟡 매매 (저위험 시 자동)
      ↓
[Chat 페이지로 이동]
      ↓
[사용자: "SK하이닉스 500만원 매수해줘"]
      ↓
[AI 분석]
 - Research → Strategy → Risk Agent 실행
 - Risk Agent: "low" 리스크 판정
      ↓
[Trading Agent 자동 실행]
 - HITL 패널 없이 바로 주문
 - approval_trade_node: automation_level=1 && risk_level=low → 자동 승인
      ↓
...
#### 시나리오 B: Copilot → Advisor 변경 (전략 승인 추가)
...
[사용자: "삼성전자 분석해줘"]
      ↓
[AI 분석]
 - Research → Strategy Agent 실행
      ↓
[HITL 패널 오픈: 전략 승인]
 "투자 전략을 승인하시겠습니까?"
 - Strategic Blueprint 표시
 - 예상 수익률, 리스크 레벨 표시
      ↓
[사용자: "수정 요청" 클릭]
      ↓
[패널 닫힘 + Chat Input 활성화]
 "삼성전자 목표 비중을 20%에서 25%로 조정해줘"
      ↓
[AI 재분석...]
```
